...
elseif RequiredScript == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:teampanels_height()
		return (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SHOW_ACCURACY"}, true) and not ShaveHUD:getSetting({"CustomHUD", "PLAYER", "KILLCOUNTER", "HIDE"}, false)) and 140 or 120
	end
	...
	if not HUDManager.CUSTOM_TEAMMATE_PANELS then	--Custom HUD compatibility
		...
		function HUDTeammate:_init_killcount()
			self._kills_panel = self._panel:panel({
				name = "kills_panel",
				visible = not ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "HIDE"}, false),
				w = 150,
				h = 20,
				x = 0,
				halign = "right"
			...
			self._kills_panel:set_rightbottom(player_panel:right(), (self._main_player or ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "TEXT"}, true)) and name_label:bottom() or name_label:top())
			local killcount_color = ShaveHUD:getColorSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "COLOR"}, "yellow")
		...
		function HUDTeammate:init_accuracy()
			...
			self._accuracy_panel = self._panel:panel({
				name = "accuracy_panel",
				visible = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SHOW_ACCURACY"}, true),
				w = 100,
				h = 20,
				x = 0,
				halign = "right"
		...
		function HUDTeammate:_update_kill_count_text()
			...
			if ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "SHOW_SPECIAL_KILLS"}, true) then
				kill_string = kill_string .. "/" .. tostring(self._kill_count_special)
			end
			if ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "SHOW_HEADSHOT_KILLS"}, true) then
			...
			if (self._main_player or ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "TEXT"}, true)) and not ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "HIDE"}, false) then
				self._max_name_panel_width = (self._kills_panel:x() + self._kill_icon:x() - 4)
				self:_truncate_name()
			end

			local color = ShaveHUD:getColorSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "COLOR"}, "yellow")
		...
		function HUDTeammate:set_state(...)
			...
			local visible = not ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "KILLCOUNTER", "HIDE"}, false) and (not self._ai or ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "KILLCOUNTER", "SHOW_BOT_KILLS"}, true))
			...
			if self._ai then
				...
				local name_label = self._panel:child("name")
				self._kills_panel:set_bottom((self._main_player or ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "TEXT"}, true)) and name_label:bottom() or name_label:top())
			end
		end
		...
		function HUDTeammate:set_accuracy(value)
			...
			if ShaveHUD:getSetting({"CustomHUD", "PLAYER", "KILLCOUNTER", "HIDE"}, false) and ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SHOW_ACCURACY"}, true) then
