...
function HUDSuspicion:animate_eye(...)
	...
	if not was_animating and self._eye_animation then
		...
		local visible = ShaveHUD:getSetting({"HUDSuspicion", "SHOW_BARS"}, true)
		...
		if ShaveHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE"}, true) and not self._text_animation then
			self._suspicion_text:set_outlines_visible(ShaveHUD:getSetting({"HUDSuspicion", "SHOW_PERCENTAGE_OUTLINE"}, true))
...
function HUDSuspicion:rescale()
	local scale = ShaveHUD:getSetting({"HUDSuspicion", "SCALE"}, 0.8)
