local print_info = print_info or function(...) ShaveHUD:print_log(string.format(...), "info") end
...
if RequiredScript == "lib/units/weapons/weapongadgetbase" then
	...
	WeaponGadgetBase.THEME_SETTINGS = {
		laser = ShaveHUD:getSetting({"GADGETS", "laser"}, nil),
		flashlight = ShaveHUD:getSetting({"GADGETS", "flashlight"}, nil)
	}
...
if string.lower(RequiredScript) == "lib/units/weapons/newraycastweaponbase" then
	...
	function NewRaycastWeaponBase:on_enabled(...)
		...
		if not self._init_laser_state and not self:is_npc() and self._assembly_complete and managers.player:current_state() == "standard" and ShaveHUD:getSetting({"GADGETS", "LASER_AUTO_ON"}, true) then
			self:_setup_laser()
...
elseif string.lower(RequiredScript) == "lib/units/beings/player/states/playerstandard" then
	...
	function PlayerStandard:_stance_entered(...)
		...
		local rotate_weapon = ShaveHUD:getSetting({"GADGETS", "SHOW_ANGLED_SIGHT"}, true) and sight_id and PlayerStandard.ANGLED_SIGHTS[sight_id]
	...
	function PlayerStandard:_check_action_weapon_gadget(t, input, ...)
		...
		local weapon_base = self._equipped_unit:base()
		local sight_id = weapon_base and weapon_base._second_sight_data and weapon_base._second_sight_data.part_id
		if input.btn_weapon_gadget_press and sight_id and PlayerStandard.ANGLED_SIGHTS[sight_id] then
