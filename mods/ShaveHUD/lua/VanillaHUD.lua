...
if ShaveHUD:getSetting({"CustomHUD", "ENABLED"}, true) then
	return
end
...
if RequiredScript == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:teammate_progress(peer_id, type_index, enabled, tweak_data_id, timer, success, ...)
		...
		if character_data then
			...
			teammate_panel:set_interact_visibility(enabled and timer and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "MIN_DURATION"}, 1) <= timer and not ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "HIDE"}, false) and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "TEXT"}, true))
		end
	end
...
elseif RequiredScript == "lib/managers/hud/hudteammate" then
	...
	function HUDTeammate:init(...)
		...
		self._condition_icon:set_color(ShaveHUD:getColorSetting({"CustomHUD", self._setting_prefix, "CONDITION_ICON_COLOR"}, "white"))
		...
		if ShaveHUD:getSetting({"CustomHUD","TEAMMATE","LATENCY"}, true) then
	...
	function HUDTeammate:set_name(name, ...)
		if not self._ai then
			if ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "TRUNCATE_TAGS"}, true) then
				name = ShaveHUD:truncateNameTag(name)
			end
			if ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "RANK"}, true) then
	...
	function HUDTeammate:_truncate_name()
		...
		if not self._ai then
			...
			name_panel:set_color(ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "USE"}, false) and ShaveHUD:getColorSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "COLOR"}, "white") or tweak_data.chat_colors[5])
		end
	...
	function HUDTeammate:_create_stamina_circle()
		...
		self._stamina_bar = radial_health_panel:bitmap({
			...
			layer = 2,
			visible = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STAMINA"}, true)
		})
	...
	function HUDTeammate:set_current_stamina(value)
		...
		self:set_stamina_meter_visibility(ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STAMINA"}, true) and not self._condition_icon:visible())
	end
	...
	function HUDTeammate:teammate_progress(enabled, tweak_data_id, timer, success, ...)
		local show = timer and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "MIN_DURATION"}, 1) <= timer and not ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "HIDE"}, false)
		teammate_progress_original(self, enabled and show, tweak_data_id, timer, success and show, ...)
		if enabled and show and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "NUMBER"}, true) then
	...
	function HUDTeammate:set_condition(icon_data, ...)
		...
		self:set_stamina_meter_visibility(not visible and ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STAMINA"}, true))
		if HUDManager.DOWNS_COUNTER_PLUGIN and self._downs_counter and self._detection_counter then
			local disabled = visible or not ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "DOWNCOUNTER"}, true) or self._ai
