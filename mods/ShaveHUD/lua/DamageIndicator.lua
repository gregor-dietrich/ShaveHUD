if not ShaveHUD:getSetting({"DamageIndicator", "ENABLED"}, true) then return end
...
if string.lower(RequiredScript) == "lib/managers/hud/hudhitdirection" then
	...
	function HUDHitDirection:_add_hit_indicator(...)
		HUDHitDirection.PANEL_SIZE = ShaveHUD:getSetting({"DamageIndicator", "SIZE"}, 150)
		if self.indicator_count < ShaveHUD:getSetting({"DamageIndicator", "MAX_AMOUNT"}, 10) then
	...
	function HUDHitDirection:_animate(indicator, data, remove_func)
		data.duration = ShaveHUD:getSetting({"DamageIndicator", "DURATION"}, 2)
		data.t = 0
	...
	function HUDHitDirection:_get_indicator_color(damage_type, t)
		if damage_type == HUDHitDirection.DAMAGE_TYPES.HEALTH then
			return ShaveHUD:getColorSetting({"DamageIndicator", "HEALTH_COLOR"}, "red")
		elseif damage_type == HUDHitDirection.DAMAGE_TYPES.ARMOUR then
			return ShaveHUD:getColorSetting({"DamageIndicator", "SHIELD_COLOR"}, "white")
		elseif damage_type == HUDHitDirection.DAMAGE_TYPES.VEHICLE then
			return ShaveHUD:getColorSetting({"DamageIndicator", "VEHICLE_COLOR"}, "yellow")
		elseif damage_type == HUDHitDirection.DAMAGE_TYPES.CRIT then
			return ShaveHUD:getColorSetting({"DamageIndicator", "CRIT_COLOR"}, "purple")
		elseif damage_type == HUDHitDirection.DAMAGE_TYPES.FRIENDLY_FIRE then
			return ShaveHUD:getColorSetting({"DamageIndicator", "FRIENDLY_FIRE_COLOR"}, "orange")
		else
			return Color(1, t, t)
		end
	end
