...
if RequiredScript == "lib/managers/hud/hudchat" then
	...
	HUDChat.LINE_HEIGHT = ShaveHUD:getSetting({"HUDChat", "LINE_HEIGHT"}, 15)			--Size of each line in chat (and hence the text size)
	HUDChat.WIDTH = ShaveHUD:getSetting({"HUDChat", "WIDTH"}, 380)						--Width of the chat window
	HUDChat.MAX_OUTPUT_LINES = ShaveHUD:getSetting({"HUDChat", "MAX_OUTPUT_LINES"}, 8)	--Number of chat lines to show
	HUDChat.MAX_INPUT_LINES = ShaveHUD:getSetting({"HUDChat", "MAX_INPUT_LINES"}, 5)		--Number of lines of text you can type
	HUDChat.MOUSE_SUPPORT = false														--For scolling and stuff. Experimental, you have been warned
	HUDChat.COLORED_BG = ShaveHUD:getSetting({"HUDChat", "COLORED_BG"}, true)			--Colorize the line bg based on the message source
	HUDChat.SCROLLBAR_ALIGN = ShaveHUD:getSetting({"HUDChat", "SCROLLBAR_ALIGN"}, 2)		--Alignment of the scroll bar (1 = left, 2 = right)
	...
	function HUDChat:_animate_fade_output()
		local wait_t = ShaveHUD:getSetting({"HUDChat", "CHAT_WAIT_TIME"}, 10)
