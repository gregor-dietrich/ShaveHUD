if RequiredScript == "lib/managers/hudmanager" then
	...
	function HUDManager:add_waypoint(id, data, ...)
		...
		if wp and wp.bitmap and wp.distance and wp.arrow and data.distance then
			local color = ShaveHUD:getColorSetting({"CustomWaypoints", "WAYPOINTS_COLOR"}, "white")
	...
	function HUDManager:setup_custom_waypoints()
		if managers.gameinfo and managers.waypoints then
			local bag_equip_settings = {
				["ammo_bag"] 		= ShaveHUD:getSetting({"CustomWaypoints", "SHOW_AMMO_BAG"}, true),
				["doc_bag"] 		= ShaveHUD:getSetting({"CustomWaypoints", "SHOW_DOC_BAG"}, true),
				["body_bag"] 		= ShaveHUD:getSetting({"CustomWaypoints", "SHOW_BODY_BAG"}, true),
				["grenade_crate"] 	= ShaveHUD:getSetting({"CustomWaypoints", "SHOW_GRENADE_CRATE"}, true),
				["first_aid_kit"] 	= ShaveHUD:getSetting({"CustomWaypoints", "SHOW_FIRST_AID_KIT"}, false),
			}
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "SHOW_SENTRIES"}, true) then
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "SHOW_ECMS"}, true) then
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "SHOW_TIMERS"}, true) then
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "SHOW_MINIONS"}, true) then
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "LOOT", "SHOW"}, true) then
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "SHOW_PAGER"}, true) then
			...
			if ShaveHUD:getSetting({"CustomWaypoints", "SHOW_SPECIAL_EQUIPMENT"}, true) then
		...
		if event == "add" then
			...
			local params = {
				...
				name = {
					type = "label",
					show = true,
					text = ShaveHUD:getCharacterName(unit_tweak, true)
				},
				kills = {
					type = "label",
					show = true,
	...
	function HUDManager:custom_waypoint_loot_clbk(event, key, data)
		...
		if event == "add" then
			if tweak_entry and not tweak_entry.is_vehicle and not tweak_entry.skip_exit_secure and (data.carry_id ~= "person" or managers.job:current_level_id() == "mad" and (data.bagged or data.unit:editor_id() ~= -1)) then
				local angle = HUDManager.CUSTOM_WAYPOINTS.DEBUGGING and 180 or ShaveHUD:getSetting({"CustomWaypoints", "LOOT", "ANGLE"}, 25)
				...
				if bag_name then
					local params = {
						unit = data.unit,
						offset = data.bagged and Vector3(0, 0, ShaveHUD:getSetting({"CustomWaypoints", "LOOT", "BAGGED_OFFSET"}, 30)) or Vector3(0, 0, ShaveHUD:getSetting({"CustomWaypoints", "LOOT", "OFFSET"}, 15)),
						...
						icon = {
							type = "icon",
							show = data.bagged and ShaveHUD:getSetting({"CustomWaypoints", "LOOT", "ICON"}, true),
							std_wp = "wp_bag",
							alpha = 0.5,
							on_minimap = true,
						},
