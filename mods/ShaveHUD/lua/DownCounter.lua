...
elseif string.lower(RequiredScript) == "lib/managers/hud/hudteammate" and not HUDManager.CUSTOM_TEAMMATE_PANELS then
	...
	Hooks:PostHook( HUDTeammate, "init", "ShaveHUD_DownCounter_HUDTeammate_init", function(self, ...)
		...
		self._downs_counter = self._health_panel:text({
			...
			layer = 2,
			visible = HUDManager.DOWNS_COUNTER_PLUGIN and ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "DOWNCOUNTER"}, true) and not self._ai or false,
		})
	...
	Hooks:PostHook( HUDTeammate, "remove_panel", "ShaveHUD_DownCounter_HUDTeammate_remove_panel", function(self, ...)
		managers.gameinfo:unregister_listener("HealthRadial_whisper_mode_listener" .. tostring(self._id), "whisper_mode", "change")
	end)

	Hooks:PostHook( HUDTeammate, "set_peer_id", "ShaveHUD_DownCounter_HUDTeammate_set_peer_id", function(self, ...)
		self:set_detection()
	end)

	Hooks:PostHook( HUDTeammate, "set_callsign", "ShaveHUD_DownCounter_HUDTeammate_set_callsign", function(self, ...)
		if self._main_player then
			self:set_detection()
		end
	end)

	Hooks:PreHook( HUDTeammate, "set_name", "ShaveHUD_DownCounter_HUDTeammate_set_name", function(self, teammate_name, ...)
	...
	function HUDTeammate:_whisper_mode_change(status)
		local disabled = self._condition_icon and self._condition_icon:visible() or not (HUDManager.DOWNS_COUNTER_PLUGIN and ShaveHUD:getSetting({"CustomHUD", self._setting_prefix, "DOWNCOUNTER"}, true)) or self._ai
