...
if not ShaveHUD:getSetting({"CustomHUD", "ENABLED"}, true) then
	return
end
...
local print_info = function(...)
	...
	ShaveHUD:print_log(unpack(params))
end
...
if RequiredScript == "lib/managers/hud/hudteammate" then
	...
	HUDTeammateCustom.SETTINGS = {
		...
		PLAYER = {
			SCALE = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SCALE"}, 1),			--Scale of all elements of the panel
			OPACITY = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "OPACITY"}, 0.85),	--Transparency/alpha of panel (1 is solid, 0 is invisible)

			NAME = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "NAME"}, false),	--Show name
			TRUNCATE_TAGS = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "TRUNCATE_TAGS"}, false),	--Truncate tags
			RANK = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "RANK"}, false),	--Show infamy/level
			CHARACTER = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "CHARACTER"}, false),	--Show character name
			LATENCY = false,	--Show latency (not used by player panel)
			STATUS = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true),	--Show health/armor/condition etc.
			CONDITION_ICON_COLOR = ShaveHUD:getColorSetting({"CustomHUD", "PLAYER", "CONDITION_ICON_COLOR"}, "white"),
			EQUIPMENT = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "EQUIPMENT"}, true),	--Show throwables, cable ties and deployables
			SPECIAL_EQUIPMENT = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SPECIAL_EQUIPMENT"}, true),	--Show special equipment/tools (keycards etc.)
			SPECIAL_EQUIPMENT_ROWS = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SPECIAL_EQUIPMENT_ROWS"}, 3),
			CALLSIGN = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "CALLSIGN"}, false),	--Show the callsign and voice chat icon
			STAMINA = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STAMINA"}, true),
			DOWNCOUNTER = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "DOWNCOUNTER"}, true),
			CARRY = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "CARRY"}, true),	--Show currently carried bag
			...
			WEAPON = {
				...
				ICON = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "ICON"}, 4) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "ICON"}, 4) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "ICON"}, 4) == 3),
				},
				NAME = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "NAME"}, 1) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "NAME"}, 1) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "NAME"}, 1) == 3),
				},
				AMMO = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "AMMO"}, 4) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "AMMO"}, 4) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "AMMO"}, 4) == 3),
					TOTAL_AMMO_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "AMMO"}, 4) == 5),	--Shows only total ammo for all weapons
				},
				FIREMODE = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "FIREMODE"}, 2) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "FIREMODE"}, 2) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "WEAPON", "FIREMODE"}, 2) == 3),
				},
			},
			...
			KILLCOUNTER = {
				...
				HIDE = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "KILLCOUNTER", "HIDE"}, false),	--Hides the kill counter
				SHOW_BOT_KILLS = false,	--Show the kill counter for criminal bots
				SHOW_SPECIAL_KILLS = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "KILLCOUNTER", "SHOW_SPECIAL_KILLS"}, true),	--Separate counter for specials
				SHOW_HEADSHOT_KILLS = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "KILLCOUNTER", "SHOW_HEADSHOT_KILLS"}, true),	--Separate counter, of how many kills were due to headshots
				COLOR = ShaveHUD:getColorSetting({"CustomHUD", "PLAYER", "KILLCOUNTER", "COLOR"}, "yellow")
			},
			SHOW_ACCURACY = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "SHOW_ACCURACY"}, true),	--Show accuracy information
		},

		TEAMMATE = {
			SCALE = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "SCALE"}, 0.8),			--Scale of all elements of the panel
			OPACITY = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "OPACITY"}, 0.85),	--Transparency/alpha of panel (1 is solid, 0 is invisible)

			NAME = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "NAME"}, true),	--Show name
			TRUNCATE_TAGS = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "TRUNCATE_TAGS"}, false),	--Truncate tags
			RANK = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "RANK"}, true),	--Show infamy/level
			CHARACTER = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "CHARACTER"}, false),	--Show character name
			LATENCY = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "LATENCY"}, true),	--Show latency (not used by player panel)
			STATUS = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "STATUS"}, true),	--Show health/armor/condition etc.
			CONDITION_ICON_COLOR = ShaveHUD:getColorSetting({"CustomHUD", "TEAMMATE", "CONDITION_ICON_COLOR"}, "white"),
			EQUIPMENT = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "EQUIPMENT"}, true),	--Show throwables, cable ties and deployables
			SPECIAL_EQUIPMENT = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "SPECIAL_EQUIPMENT"}, true),	--Show special equipment/tools (keycards etc.)
			SPECIAL_EQUIPMENT_ROWS = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "SPECIAL_EQUIPMENT_ROWS"}, 3),
			CALLSIGN = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "CALLSIGN"}, false),	--Show the callsign and voice chat icon
			STAMINA = false,
			DOWNCOUNTER = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "DOWNCOUNTER"}, true),
			CARRY = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "CARRY"}, true),	--Show currently carried bag
			BUILD = {	--Show perk deck and number of skills acquired in each tree (not used by player)
				--Pick max one
				HIDE = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "BUILD", "HIDE"}, false),	--Don't show build at all
				DURATION = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "BUILD", "DURATION"}, 15) > 0 and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "BUILD", "DURATION"}, 15) or nil,	--Time in seconds to show the build from when player joins. Information is hidden when duration has expired, or never removed if value is nil/undefined
			},
			WEAPON = {
				--Show/hide various elements of the weapons panels.
				--HIDE option hides the element. SELECTED_ONLY shows only if the weapon is currently selected, UNSELECTED_ONLY the reverse
				--Pick max *one* setting for each element or results are undefined
				ICON = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "ICON"}, 4) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "ICON"}, 4) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "ICON"}, 4) == 3),
				},
				NAME = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "NAME"}, 1) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "NAME"}, 1) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "NAME"}, 1) == 3),
				},
				AMMO = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "AMMO"}, 4) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "AMMO"}, 4) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "AMMO"}, 4) == 3),
					TOTAL_AMMO_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "AMMO"}, 4) == 5),	--Shows only total ammo for all weapons
				},
				FIREMODE = {
					HIDE = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "FIREMODE"}, 1) == 1),
					SELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "FIREMODE"}, 1) == 2),
					UNSELECTED_ONLY = (ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "WEAPON", "FIREMODE"}, 1) == 3),
				},
			},
			INTERACTION = {	--(Interaction display only used by teammates, included for reference)
				HIDE = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "HIDE"}, false),	--Hides the interaction activity/time/progress
				MIN_DURATION = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "INTERACTION", "MIN_DURATION"}, 1),	--Shows the interaction display only if interaction duration in seconds exceeds this threshold
			},
			KILLCOUNTER = {
				--Requires external plugin to be loaded, else will be disabled no matter what
				HIDE = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "KILLCOUNTER", "HIDE"}, false),	--Hides the kill counter
				SHOW_BOT_KILLS = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "KILLCOUNTER", "SHOW_BOT_KILLS"}, true),	--Show the kill counter for criminal bots
				SHOW_SPECIAL_KILLS = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "KILLCOUNTER", "SHOW_SPECIAL_KILLS"}, true),	--Separate counter for specials
				SHOW_HEADSHOT_KILLS = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "KILLCOUNTER", "SHOW_HEADSHOT_KILLS"}, true),	--Separate counter, of how many kills were due to headshots
				COLOR = ShaveHUD:getColorSetting({"CustomHUD", "TEAMMATE", "KILLCOUNTER", "COLOR"}, "yellow")
			},
...
--[[
	...
	function PlayerInfoComponent.PlayerInfo:set_id(id)
		self._id = id
		self:_set_text_color((id == 5 and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "USE"}, false)) and ShaveHUD:getColorSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "COLOR"}, "white") or tweak_data.chat_colors[id])
	end
	...
	function PlayerInfoComponent.PlayerInfo:set_name(name)
		if name then
			if self._settings.TRUNCATE_TAGS and not self._is_ai then
				name = ShaveHUD:truncateNameTag(name)
			end
...
--[[
	...
	function PlayerInfoComponent.Interaction:stop(success)
		if self:visible() then
			...
			self._text:set_text(managers.localization:to_upper_text(success and "shavehud_customhud_interaction_done" or "shavehud_customhud_interaction_aborted"))
			--self._interaction:set_enabled("active", false)
			--self:arrange()
		end
	end
...
if RequiredScript == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:_create_teammates_panel(hud, ...)
		...
		for i = 1, num_panels do
			...
			if is_player then
				align = ShaveHUD:getSetting({"CustomHUD", "PLAYER", "POSITION"}, 2) < 3 and "left" or "right"
			elseif j <= 7 then
				align = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "POSITION"}, 1) < 3 and "left" or "right"
			else
				align = ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "POSITION"}, 1) > 1 and "right" or "left"
			end
	...
	function HUDManager:arrange_teammate_panels()
		...
		if player_hud and player_panel then
			...
			local player_pos = math.clamp(ShaveHUD:getSetting({"CustomHUD", "PLAYER", "POSITION"}, 2), 1, 3)
			...
			local teammate_pos = { math.clamp(ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "POSITION"}, 1), 1, 3) }
