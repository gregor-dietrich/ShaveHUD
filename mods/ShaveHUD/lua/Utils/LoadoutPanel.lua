...
--[[
	if status and managers.chat then	-- TODO: Needs localization
		local message = string.format("[ShaveHUD] %s is cheating: %s", self:get_peer():name(), reason or "N/A")
	...
	if success then
		...
		if gameStart then
			...
			if hoursStart and hoursEnd and ((hoursEnd - hoursStart) > 2) then
				...
				text = managers.localization:text("shavehud_loadout_hours_played", { HOURS = playtime })
			end
		else
