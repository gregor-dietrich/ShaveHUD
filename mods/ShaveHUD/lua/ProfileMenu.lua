local PROFILE_MENU_ID = "shavehud_profile_switch_node_menu"
...
if RequiredScript == "lib/managers/menumanager" then
	...
	Hooks:Add("MenuManagerBuildCustomMenus", "ShaveHUD_MenuManager_BuildProfileMenu", function( menu_manager, nodes )
	...
	function ProfileSwitchInitiator:modify_node(node, data)
		node:clean_items()
		self:create_divider(node, "title", "shavehud_profile_switch_title_profiles", nil, tweak_data.screen_colors.text)
...
elseif RequiredScript == "lib/managers/menu/renderers/menunodeskillswitchgui" then
	...
	function MenuNodeProfileSwitchGui:_setup_item_panel(...)
		...
		if alive(self.title_text) then
			self.title_text:set_text(managers.localization:to_upper_text("shavehud_profile_switch_title"))
		end
...
elseif RequiredScript == "lib/managers/multiprofilemanager" then
    ...
    function MultiProfileManager:open_quick_select(...)
        if ShaveHUD:getSetting({"CrewLoadout", "REPLACE_PROFILE_MENU"}, true) then
...
elseif RequiredScript == "lib/managers/menu/multiprofileitemgui" then
	...
	function MultiProfileItemGui:init(...)
		...
		self._max_length = ShaveHUD:getTweakEntry("MAX_PROFILE_NAME_LENGTH", "number", 20)
	end
