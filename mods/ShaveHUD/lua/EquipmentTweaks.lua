if string.lower(RequiredScript) == "lib/units/weapons/sentrygunweapon" then
	...
	function SentryGunWeapon:post_init()
		...
		if ShaveHUD:getSetting({"EQUIPMENT", "SENTRY_AUTO_AP"}, true) and enable_ap then
...
elseif string.lower(RequiredScript) == "lib/units/equipment/ecm_jammer/ecmjammerbase" then
	...
	function ECMJammerBase:setup(...)
		setup_original(self, ...)
		if ShaveHUD:getSetting({"EQUIPMENT", "ECM_FEEDBACK_STEALTH_DISABLED"}, true) and managers.groupai:state():whisper_mode() then
	...
	function ECMJammerBase:contour_interaction(...)
		if not (managers.groupai:state():whisper_mode() and ShaveHUD:getSetting({"EQUIPMENT", "ECM_FEEDBACK_STEALTH_DISABLED"}, true)) then
...
elseif string.lower(RequiredScript) == "lib/units/interactions/interactionext" then
	BaseInteractionExt.SHAPED_CHARGE_TIMEOUT = ShaveHUD:getTweakEntry("STEALTH_SHAPED_CHARGE_TIMEOUT", "number", 0.25)		--Timeout for 2 InteractKey pushes, to prevent accidents in stealth
	BaseInteractionExt.KEYCARD_DOORS_TIMEOUT = ShaveHUD:getTweakEntry("KEYCARD_DOORS_TIMEOUT", "number", 0.25)		--Timeout for 2 InteractKey pushes, to prevent accidents in hoxton breakout day 2
	...
	function BaseInteractionExt:interact_start(player, data, ...)
		...
		if ShaveHUD:getSetting({"EQUIPMENT", "SHAPED_CHARGE_STEALTH_DISABLED"}, true) and managers.groupai:state():whisper_mode()
		...
		if ShaveHUD:getSetting({"EQUIPMENT", "KEYCARD_DOORS_DISABLED"}, true)
