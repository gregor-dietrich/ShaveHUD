...
if not (ShaveHUD and ShaveHUD:getSetting({"HUDList", "ENABLED"}, true)) then return end
...
if string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:_setup_player_info_hud_pd2(...)
		...
		if managers.gameinfo then
			...
			ShaveHUD:print_log("(HUDList) GameInfoManager not present!", "error")
		end
	end
	...
	HUDListManager.ListOptions = {
		...
		right_list_scale 				= ShaveHUD:getSetting({"HUDList", "right_list_scale"}, 1),   	--Size scale of right list
		left_list_scale 				= ShaveHUD:getSetting({"HUDList", "left_list_scale"}, 1),    	--Size scale of left list
		buff_list_scale 				= ShaveHUD:getSetting({"HUDList", "buff_list_scale"}, 1),    	--Size scale of buff list
		right_list_progress_alpha 		= ShaveHUD:getSetting({"HUDList", "right_list_progress_alpha"}, 0.4),
		left_list_progress_alpha 		= ShaveHUD:getSetting({"HUDList", "left_list_progress_alpha"}, 0.4),
		buff_list_progress_alpha 		= ShaveHUD:getSetting({"HUDList", "buff_list_progress_alpha"}, 1.0),
		
		timers_in_seconds 				= ShaveHUD:getSetting({"HUDList", "timers_in_seconds"}, false),

		--Left side list
		show_timers 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_timers"}, true),     				--Drills, time locks, hacking etc.
		show_ammo_bags 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_ammo_bags"}, true),
		show_doc_bags 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_doc_bags"}, true),
		show_first_aid_kits				= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_first_aid_kits"}, false),
		show_body_bags 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_body_bags"}, true),
		show_grenade_crates 			= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_grenade_crates"}, true),
		show_sentries 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_sentries"}, true),   				--Deployable sentries
		show_ecms 						= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_ecms"}, true),       				--Active ECMs
		show_ecm_retrigger 				= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_ecm_retrigger"}, true),      		--Countdown for player owned ECM feedback retrigger delay
		show_minions 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_minions"}, true),    				--Converted enemies, type and health
			show_own_minions_only 		= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_own_minions_only"}, true),			--Only show player-owned minions
		show_pagers 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_pagers"}, true),     				--Show currently active pagers
		show_tape_loop 					= ShaveHUD:getSetting({"HUDList", "LEFT_LIST", "show_tape_loop"}, true),  				--Show active tape loop duration

		--Right side list
		show_enemies 					= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_enemies"}, true),            		--Currently spawned enemies
			aggregate_enemies 			= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "aggregate_enemies"}, false),      			--Aggregate all enemies into a single item
		show_turrets 					= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_turrets"}, true),    				--Show active SWAT turrets
		show_civilians 					= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_civilians"}, true),  				--Currently spawned, untied civs
		show_hostages 					= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_hostages"}, true),   				--Currently tied civilian and dominated cops
			aggregate_hostages 			= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "aggregate_hostages"}, false),															--Aggregate all hostages into a single item
		show_minion_count 				= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_minion_count"}, true),       		--Current number of jokered enemies
		show_pager_count 				= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_pager_count"}, true),        		--Show number of triggered pagers (only counts pagers triggered while you were present)
		show_cam_count					= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_cam_count"}, true),
		show_bodybags_count				= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_bodybags_count"}, true),
		show_corpse_count				= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_corpse_count"}, true),
		show_loot 						= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_loot"}, true),       				--Show spawned and active loot bags/piles (may not be shown if certain mission parameters has not been met)
			aggregate_loot 				= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "aggregate_loot"}, false), 					--Aggregate all loot into a single item
			separate_bagged_loot 		= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "separate_bagged_loot"}, true),     		--Show bagged/unbagged loot as separate values
			show_potential_loot			= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_potential_loot"}, false),
		show_special_pickups 			= ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "show_special_pickups"}, true),    		--Show number of special equipment/items

		--Buff list
		show_buffs 						= ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "show_buffs"}, true),       				--Active effects (buffs/debuffs). Also see HUDList.BuffItemBase.IGNORED_BUFFS table to ignore specific buffs that you don't want listed, or enable some of those not shown by default

		list_color 						= ShaveHUD:getColorSetting({"HUDList", "list_color"}, "white"),
		list_color_bg 					= ShaveHUD:getColorSetting({"HUDList", "list_color_bg"}, "black"),
		civilian_color 					= ShaveHUD:getColorSetting({"HUDList", "civilian_color"}, "white"),
		hostage_color 					= ShaveHUD:getColorSetting({"HUDList", "civilian_color"}, "white"),
		thug_color 						= ShaveHUD:getColorSetting({"HUDList", "thug_color"}, "white"),
		enemy_color 					= ShaveHUD:getColorSetting({"HUDList", "enemy_color"}, "white"),
		guard_color 					= ShaveHUD:getColorSetting({"HUDList", "enemy_color"}, "white"),
		special_color 					= ShaveHUD:getColorSetting({"HUDList", "special_color"}, "white"),
		turret_color 					= ShaveHUD:getColorSetting({"HUDList", "special_color"}, "white"),
	}
	...
	HUDListManager.UNIT_TYPES = {
		cop = 						{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_cop" 					},
		cop_female = 				{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_cop" 					},
		fbi = 						{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_fbi" 					},
		swat = 						{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_swat" 					},
		heavy_swat = 				{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_heavy_swat" 				},
        heavy_swat_sniper =         { type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_heavy_swat_sniper"   	},
		fbi_swat = 					{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_swat" 					},
		fbi_heavy_swat = 			{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_heavy_swat" 				},
		city_swat = 				{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_city_swat" 				},
		security = 					{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_security" 				},
		security_undominatable = 	{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_security" 				},
		security_mex = 				{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_security" 				},
		security_mex_no_pager = 	{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_security" 				},
		gensec = 					{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_gensec" 					},
		bolivian_indoors =			{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_bolivian_security" 		},
		bolivian_indoors_mex =		{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_bolivian_security_mex" 	},
		bolivian =					{ type_id = "thug",			category = "enemies",	long_name = "shavehud_enemy_bolivian_thug" 			},
		gangster = 					{ type_id = "thug",			category = "enemies",	long_name = "shavehud_enemy_gangster" 				},
		mobster = 					{ type_id = "thug",			category = "enemies",	long_name = "shavehud_enemy_mobster" 				},
		biker = 					{ type_id = "thug",			category = "enemies",	long_name = "shavehud_enemy_biker" 					},
		biker_escape = 				{ type_id = "thug",			category = "enemies",	long_name = "shavehud_enemy_biker" 					},
		tank = 						{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank" 					},
		tank_hw = 					{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank_hw" 				},
		tank_medic = 				{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank_medic" 				},
		tank_mini = 				{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank_mini" 				},
		spooc = 					{ type_id = "spooc",		category = "enemies",	long_name = "shavehud_enemy_spook" 					},
		shadow_spooc = 				{ type_id = "spooc",		category = "enemies",	long_name = "shavehud_enemy_shadow_spook" 			},
		taser = 					{ type_id = "taser",		category = "enemies",	long_name = "shavehud_enemy_taser" 					},
		shield = 					{ type_id = "shield",		category = "enemies",	long_name = "shavehud_enemy_shield" 					},
		sniper = 					{ type_id = "sniper",		category = "enemies",	long_name = "shavehud_enemy_sniper" 					},
		medic = 					{ type_id = "medic",		category = "enemies",	long_name = "shavehud_enemy_medic" 					},
		biker_boss = 				{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_biker_boss" 				},
		chavez_boss = 				{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_chavez_boss" 			},
		drug_lord_boss =			{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_druglord_boss" 			},
		drug_lord_boss_stealth =	{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_druglord_boss_stealth" 	},
		hector_boss = 				{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_hector_boss" 			},
		hector_boss_no_armor = 		{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_hector_boss_no_armor" 	},
		mobster_boss = 				{ type_id = "thug_boss",	category = "enemies",	long_name = "shavehud_enemy_mobster_boss" 			},
		phalanx_vip = 				{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_phalanx_vip" 			},
		phalanx_minion = 			{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_phalanx_minion" 			},
		civilian = 					{ type_id = "civ",			category = "civilians",	long_name = "shavehud_enemy_civilian" 				},
		civilian_female = 			{ type_id = "civ",			category = "civilians",	long_name = "shavehud_enemy_civilian" 				},
		civilian_mariachi = 		{ type_id = "civ",			category = "civilians",	long_name = "shavehud_enemy_civilian" 				},
		bank_manager = 				{ type_id = "civ",			category = "civilians",	long_name = "shavehud_enemy_bank_manager" 			},
		--drunk_pilot = 			{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_drunk_pilot" 			},	--White x-Mas
		--escort = 					{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_escort" 					},	--?
		--old_hoxton_mission = 		{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_old_hoxton_mission" 		},	--Hox Breakout / BtM (Locke)
		--inside_man = 				{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_inside_man" 				},	--FWB
		--boris = 					{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_boris" 					},	--Goat Sim Day 2
		--escort_undercover = 		{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_escort_undercover" 		},	--Taxman, Undercover + Matt, Heat Street
		--escort_chinese_prisoner = { type_id = "unique", 		category = "civilians", long_name = "shavehud_enemy_escort_chinese_prisoner" },	--Kazo, Green Bridge
		--spa_vip = 				{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_spa_vip" 				},	--Charon, Wick Heist
		--spa_vip_hurt = 			{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_spa_vip_hurt" 			},	--Charon, Wick Heist
		--escort_criminal = 		{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_escort_criminal" 		},	--???, Breakfast

		--Custom unit definitions
		--mechanic = 				{ type_id = "unique",		category = "civilians",	long_name = "shavehud_enemy_biker_mechanic" 			},	-- Mechanic, Biker Heist
		turret = 					{ type_id = "turret",		category = "turrets",	long_name = "shavehud_enemy_swat_van" 				},
		...
		boom = 						{ type_id = "grenadier",	category = "enemies",	long_name = "shavehud_enemy_boom" 					},
		omnia_lpf = 				{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_omnia_lpf" 				},
		summers = 					{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_summers" 				},
		boom_summers = 				{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_boom_summers" 			},
		taser_summers = 			{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_taser_summers" 			},
		medic_summers = 			{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_medic_summers" 			},
		spring = 					{ type_id = "phalanx",		category = "enemies",	long_name = "shavehud_enemy_spring" 					},
		fbi_vet = 					{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_fbi_vet" 				},

		--Crackdown Enemies
		deathvox_lightar = 			{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_deathvox_light" },
		deathvox_heavyar = 			{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_deathvox_heavy" },
		deathvox_lightshot = 		{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_deathvox_light" },
		deathvox_heavyshot = 		{ type_id = "cop",			category = "enemies",	long_name = "shavehud_enemy_deathvox_heavy" },
		deathvox_greendozer = 		{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank" },
		deathvox_blackdozer = 		{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank" },
		deathvox_lmgdozer = 		{ type_id = "tank",			category = "enemies",	long_name = "shavehud_enemy_tank" },
		deathvox_medicdozer =		{ type_id = "tank_med",		category = "enemies",	long_name = "shavehud_enemy_tank_medic" },
		deathvox_cloaker =			{ type_id = "spooc",		category = "enemies",	long_name = "shavehud_enemy_spooc" },
		deathvox_taser =			{ type_id = "taser",		category = "enemies",	long_name = "shavehud_enemy_taser" },
		deathvox_shield =			{ type_id = "shield",		category = "enemies",	long_name = "shavehud_enemy_shield" },
		deathvox_sniper =			{ type_id = "sniper",		category = "enemies",	long_name = "shavehud_enemy_sniper" },
		deathvox_medic =			{ type_id = "medic",		category = "enemies",	long_name = "shavehud_enemy_medic" },
		deathvox_grenadier =		{ type_id = "grenadier",	category = "enemies",	long_name = "shavehud_enemy_boom" },
		deathvox_guard =			{ type_id = "security",		category = "enemies",	long_name = "shavehud_enemy_security" },
	}
	...
	function HUDListManager:_buff_event(event, id, data)
		ShaveHUD:print_log("(HUDList) _buff_event(%s, %s)", tostring(event), tostring(id), "info")
		...
		for _, item in ipairs(items) do
			if item[event] then
				...
				ShaveHUD:print_log("HUDList) _buff_event: No matching function for event %s for buff %s", tostring(event), tostring(id), "warning")
			end
		end
	...
	HUDList.SpecialPickupItem.MAP = {
		crowbar =					{ hudpickups = { 0, 64, 32, 32 }, 												priority = 1, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		keycard =					{ hudpickups = { 32, 0, 32, 32 }, 												priority = 1, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		planks =					{ hudpickups = { 0, 32, 32, 32 }, 												priority = 2, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		meth_ingredients =			{ waypoints  = { 192, 32, 32, 32 }, 											priority = 2, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		blowtorch = 				{ hudpickups = { 96, 192, 32, 32 }, 											priority = 1, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		thermite = 					{ hudpickups = { 64, 64, 32, 32 }, 												priority = 1, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		c4 = 						{ hudicons	 = { 36, 242, 32, 32 }, 											priority = 1, category = "mission_pickups", ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "mission_pickups"}, true) 	},
		small_loot = 				{ hudpickups = { 32, 224, 32, 32}, 												priority = 3, category = "valuables", 		ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "valuables"}, true) 		},
		briefcase = 				{ hudpickups = { 96, 224, 32, 32}, 												priority = 4, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		courier = 					{ texture = "guis/dlcs/gage_pack_jobs/textures/pd2/endscreen/gage_assignment", 	priority = 3, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		gage_case = 				{ skills 	 = { 1, 0 }, 														priority = 3, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		gage_key = 					{ hudpickups = { 32, 64, 32, 32 }, 												priority = 3, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		paycheck_masks = 			{ hudpickups = { 128, 32, 32, 32 }, 											priority = 4, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		secret_item =				{ waypoints  = { 96, 64, 32, 32 }, 												priority = 4, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		rings = 					{ texture = "guis/textures/pd2/level_ring_small", w_ratio = 0.5, h_ratio = 0.5, priority = 4, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		poster = 					{ hudpickups = { 96, 96, 32, 32 }, 												priority = 4, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
		handcuffs = 				{ hud_icons  = {294,469, 40, 40 }, 												priority = 4, category = "collectables", 	ignore = not ShaveHUD:getSetting({"HUDList", "RIGHT_LIST", "SHOW_PICKUP_CATEGORIES", "collectables"}, true) 		},
	}
	...
	HUDList.LootItem.MAP = {
		...
		armor =			{ text = "shavehud_hudlist_loot_armor", 		priority = 1 }, -- Shaddow Raid
		...
		bomb =			{ text = "shavehud_hudlist_loot_bomb", 		priority = 1 },	-- Bomb Forest & Dockyard, Murky Station EMP
		coke =			{ text = "hud_carry_coke", 					priority = 1 },
		dentist =		{ text = "???", no_localize = true, 		priority = 1 },	-- Golden Grin
		diamond = 		{ text = "shavehud_hudlist_loot_diamond", 	priority = 1 },	-- The Diamond/Diamond Heist Red Diamond
		diamonds =		{ text = "hud_carry_diamonds_dah", 			priority = 1 },	-- The Diamond Heist
		drone_ctrl = 	{ text = "hud_carry_helmet", 				priority = 1 },	-- Biker Heist
		egg = 			{ text = "shavehud_hudlist_loot_egg", 		priority = 1 },	-- San Martin Bank
		evidence =		{ text = "shavehud_hudlist_loot_evidence", 	priority = 1 },	-- Hoxton revenge
		...
		shoes = 		{ text = "shavehud_hudlist_loot_shoes", 		priority = 1 },	-- Stealing Xmas
		toast =			{ text = "shavehud_hudlist_loot_toast", 		priority = 1 },	-- White Xmas
		toothbrush = 	{ text = "shavehud_hudlist_loot_toothbrush", priority = 1 },	-- Panic Room
		toy = 			{ text = "shavehud_hudlist_loot_toy", 		priority = 1 },	-- Stealing Xmas
		treasure = 		{ text = "shavehud_hudlist_loot_treasure", 	priority = 1 },	-- San Martin Bank
		turret =		{ text = "hud_carry_turret", 				priority = 1 },	-- Transport: Train
		vr = 			{ text = "shavehud_hudlist_loot_vr", 		priority = 1 },	-- Stealing Xmas
		warhead =		{ text = "hud_carry_warhead", 				priority = 1 },	-- Meltdown
		weapon =		{ text = "shavehud_hudlist_loot_weapon", 	priority = 1 },
		wine = 			{ text = "hud_carry_wine", 					priority = 1 },	-- Stealing Xmas
		body = 			{ text = "hud_carry_person", 				priority = 1 },	-- Boiling point
		crate = 		{ text = "shavehud_hudlist_loot_crate", 		priority = 2, no_separate = true },
		xmas_present = 	{ text = "hud_carry_present", 				priority = 2, no_separate = true },	-- White Xmas
		shopping_bag = 	{ text = "shavehud_hudlist_loot_bag", 		priority = 2, no_separate = true },	-- White Xmas
		showcase = 		{ text = "shavehud_hudlist_showcase", 		priority = 2, no_separate = true },	-- Diamond heist + Diamond Museum
	}
	...
	HUDList.TimerItem.DEVICE_TYPES = {
		digital 		= { class = "TimerItem", 			name = "shavehud_hudlist_device_timer" 	},
		drill 			= { class = "UpgradeableTimerItem", name = "shavehud_hudlist_device_drill" 	},
		drill_noupgrade	= { class = "TimerItem", 			name = "shavehud_hudlist_device_drill" 	},
		saw 			= { class = "UpgradeableTimerItem", name = "shavehud_hudlist_device_saw" 	},
		saw_noupgrade 	= { class = "TimerItem", 			name = "shavehud_hudlist_device_saw" 	},
		hack 			= { class = "TimerItem", 			name = "shavehud_hudlist_device_hack" 	},
		timer 			= { class = "TimerItem", 			name = "shavehud_hudlist_device_timer" 	},
		securitylock 	= { class = "SecurityTimerItem", 	name = "shavehud_hudlist_device_security"},
	}
...
--[[
	...
	function HUDList.SecurityTimerItem:_update_bar_text()
		local text = managers.localization:text("shavehud_hudlist_device_security_bar", { CURRENT = self._bars.current or 1, TOTAL = self._bars.total or 3})
	...
	HUDList.BuffItemBase.MAP = {
		...
		aggressive_reload_aced = {
			...
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "aggressive_reload_aced"}, true),
		},
		ammo_efficiency = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "ammo_efficiency"}, true)
		},
		armor_break_invulnerable = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "armor_break_invulnerable"}, true),
		},
		berserker = {
			...
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "berserker"}, true),
		},
		biker = {
			perks = {0, 0},
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "biker"}, true),
		},
		bloodthirst_aced = {
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ace_icon = true,
			title = "shavehud_hudlist_buff_aced",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "bloodthirst_aced"}, true),
		},
		bloodthirst_basic = {
			skills_new = tweak_data.skilltree.skills.bloodthirst.icon_xy,
			class = "BuffItemBase",
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			title = "shavehud_hudlist_buff_basic",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "bloodthirst_basic"}, false),
		},
		bullet_storm = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "bullet_storm"}, true),
		},
		chico_injector = {
			perks = {0, 0},
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "chico_injector"}, false) and (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or ShaveHUD:getSetting({"CustomHUD", "ENABLED"}, false)),
		},
		close_contact = {
			perks = {5, 4},
			class = "TimedBuffItem",
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "close_contact"}, true),
		},
		combat_medic = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "combat_medic"}, true),
		},
		combat_medic_passive = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "combat_medic_passive"}, false),
		},
		delayed_damage = {
			perks = {3, 0},
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			show_value = "-%.0f",
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "delayed_damage"}, true),
		},
		desperado = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "desperado"}, true),
		},
		die_hard = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "die_hard"}, false),
		},
		dire_need = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "GHOST_BUFFS", "dire_need"}, true),
		},
		frenzy = {
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			show_value = HUDList.BuffItemBase.VALUE_FUNC.IN_PERCENT,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "frenzy"}, false),
		},
		grinder = {
			perks = {4, 6},
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "grinder"}, true),
		},
		hostage_situation = {
			perks = {0, 1},
			class = "BuffItemBase",
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "hostage_situation"}, false),
		},
		hostage_taker = {
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			invert_timers = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "hostage_taker"}, false),
		},
		inspire = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "inspire"}, true),
		},
		lock_n_load = {
			...
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "TECHNICIAN_BUFFS", "lock_n_load"}, true),
		},
		maniac = {
			perks = {0, 0},
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			show_value = "-%.1f",
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "maniac"}, false) and (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or ShaveHUD:getSetting({"CustomHUD", "ENABLED"}, false)),
		},
		messiah = {
			...
			priority = 3,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "messiah"}, true)
		},
		melee_stack_damage = {
			perks = {5, 4},
			class = "TimedBuffItem",
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "melee_stack_damage"}, false),
		},
		muscle_regen = {
			perks = { 4, 1 },
			class = "TimedBuffItem",
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			invert_timers = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "muscle_regen"}, false),
		},
		overdog = {
			perks = {6, 4},
			class = "TimedBuffItem",
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "overdog"}, false)
		},
		overkill = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "overkill"}, false),
		},
		painkiller = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "painkiller"}, false),
		},
		partner_in_crime = {
			...
			priority = 3,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "partner_in_crime"}, false),
		},
		pocket_ecm_jammer = {
			perks = {0, 0},
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "pocket_ecm_jammer"}, true),
		},
		pocket_ecm_kill_dodge = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "pocket_ecm_kill_dodge"}, false),
		},
		running_from_death = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "running_from_death"}, true),
		},
		quick_fix = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "quick_fix"}, false),
		},
		second_wind = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "GHOST_BUFFS", "second_wind"}, true),
		},
		sicario_dodge = {
			perks = {1, 0},
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			show_value = HUDList.BuffItemBase.VALUE_FUNC.IN_PERCENT,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "sicario_dodge"}, true),
		},
		sixth_sense = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "GHOST_BUFFS", "sixth_sense"}, true),
		},
		smoke_screen_grenade = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "smoke_screen_grenade"}, true),
		},
		swan_song = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "swan_song"}, false) and (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or ShaveHUD:getSetting({"CustomHUD", "ENABLED"}, false)),
		},
		tag_team = {
			perks = {0, 0},
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			show_value = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "tag_team"}, true) and (ShaveHUD:getSetting({"CustomHUD", "PLAYER", "STATUS"}, true) or ShaveHUD:getSetting({"CustomHUD", "ENABLED"}, false)),
		},
		tooth_and_claw = {
			perks = {0, 3},
			class = "TimedBuffItem",
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "tooth_and_claw"}, true),
		},
		trigger_happy = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "trigger_happy"}, false),
		},
		underdog = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "underdog"}, false),
		},
		unseen_strike = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "GHOST_BUFFS", "unseen_strike"}, true),
		},
		up_you_go = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "FUGITIVE_BUFFS", "up_you_go"}, false),
		},
		uppers = {
			...
			priority = 4,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "uppers"}, true),
		},
		yakuza = {
			perks = {2, 7},
			class = "BuffItemBase",
			...
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			show_value = HUDList.BuffItemBase.VALUE_FUNC.IN_PERCENT,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "yakuza"}, false),
		},
		...
		anarchist_armor_recovery_debuff = {
			...
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "anarchist_armor_recovery_debuff"}, true),
		},
		ammo_give_out_debuff = {
			...
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "ammo_give_out_debuff"}, true),
		},
		...
		bullseye_debuff = {
			...
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "bullseye_debuff"}, true),
		},
		grinder_debuff = {
			perks = {4, 6},
			class = "TimedBuffItem",
		...
		inspire_debuff = {
			...
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			title = "shavehud_hudlist_buff_boost",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "inspire_debuff"}, true),
		},
		inspire_revive_debuff = {
			...
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ace_icon = true,
			title = "shavehud_hudlist_buff_revive",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "inspire_revive_debuff"}, true),
		},
		life_drain_debuff = {
			perks = {7, 4},
			class = "TimedBuffItem",
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "life_drain_debuff"}, true),
		},
		maniac_debuff = {
			perks = {0, 0},
			texture_bundle_folder = "coco",
		...
		medical_supplies_debuff = {
			...
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "medical_supplies_debuff"}, true),
		},
		...
		sociopath_debuff = {
			...
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "sociopath_debuff"}, true),
		},
		tag_team_debuff = {
			perks = {0, 0},
			texture_bundle_folder = "ecp",
		...
		damage_control_debuff = {
			...
			priority = 8,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "damage_control_debuff"}, false),
		},
		...
		armorer = {
			...
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "armorer"}, true),
		},
		bulletproof = {						--TODO: Needs new icon (Faster Team armor recovery)
			...
			priority = 1,
			color = HUDList.BuffItemBase.ICON_COLOR.TEAM,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "ENFORCER_BUFFS", "bulletproof"}, true),
		},
		crew_chief = {
			perks = {2, 0},
			class = "TeamBuffItem",
			priority = 1,
			color = HUDList.BuffItemBase.ICON_COLOR.TEAM,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "PERK_BUFFS", "crew_chief"}, true),
		},
		endurance = {
		...
		forced_friendship = {
			...
			priority = 1,
			color = HUDList.BuffItemBase.ICON_COLOR.TEAM,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "MASTERMIND_BUFFS", "forced_friendship"}, true),
		},
		...
		invulnerable_buff = {
			...
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "GAGE_BOOSTS", "invulnerable_buff"}, true),
		},
		life_steal_debuff = {
			...
			priority = 10,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "GAGE_BOOSTS", "life_steal_debuff"}, true),
		},
		...
		crew_inspire_debuff = {
			...
			class = "TimedBuffItem",
			priority = 10,
			title = "shavehud_hudlist_buff_crew_inspire_debuff",
			localized = true,
			color = HUDList.BuffItemBase.ICON_COLOR.DEBUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "AI_SKILLS", "crew_inspire_debuff"}, true),
		},
		crew_throwable_regen = {
			...
			priority = 10,
			color = HUDList.BuffItemBase.ICON_COLOR.BUFF,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "AI_SKILLS", "crew_throwable_regen"}, true),
		},
		crew_health_regen = {
			hud_tweak = "skill_5",
			class = "TimedBuffItem",
			...
			color = HUDList.BuffItemBase.ICON_COLOR.BUFF,
			invert_timers = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "AI_SKILLS", "crew_health_regen"}, true),
		},
		...
		damage_increase = {
			...
			priority = 2,
			color = Color(1, 1, 0),
			title = "shavehud_hudlist_buff_dmg_inc",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "damage_increase"}, true),
		},
		damage_reduction = {
			...
			priority = 2,
			color = Color(0, 1, 1),
			title = "shavehud_hudlist_buff_dmg_dec",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "damage_reduction"}, true),
		},
		melee_damage_increase = {
			...
			priority = 2,
			color = Color(1, 0, 1),
			title = "shavehud_hudlist_buff_mdmg_inc",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "melee_damage_increase"}, true),
		},
		passive_health_regen = {
			...
			title = "shavehud_hudlist_buff_phealth_reg",
			localized = true,
			invert_timers = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "passive_health_regen"}, true),
		},
		total_dodge_chance = {	--missing some skills:
			...
			priority = 2,
			color = Color(1, 0.5, 0),
			title = "shavehud_hudlist_buff_tot_dodge",
			localized = true,
			ignore = not ShaveHUD:getSetting({"HUDList", "BUFF_LIST", "total_dodge_chance"}, true),
		},
		...
		weapon_charge = {
			texture = "guis/textures/shavehud/hudlist/weapon_charge",
			class = "TimedBuffItem",
			priority = 15,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = false,
		...
		melee_charge = {
			...
			priority = 15,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = ShaveHUD:getSetting({"INTERACTION", "SHOW_MELEE"}, true)
		},
		reload = {
			--skills_new = tweak_data.skilltree.skills.speedy_reload.icon_xy,
			skills = {0, 9},
			...
			priority = 15,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = ShaveHUD:getSetting({"INTERACTION", "SHOW_RELOAD"}, true)
		},
		interact = {
			...
			priority = 15,
			color = HUDList.BuffItemBase.ICON_COLOR.STANDARD,
			ignore = (ShaveHUD:getSetting({"INTERACTION", "SHOW_CIRCLE"}, true) or ShaveHUD:getSetting({"INTERACTION", "SHOW_TIME_REMAINING"}, true))
		},
		interact_debuff = {
			--skills_new = tweak_data.skilltree.skills.second_chances.icon_xy,
			texture = "guis/textures/pd2/skilltree/drillgui_icon_faster",
	...
	function HUDList.CompositeBuff:set_value(id, data)
		if self._member_buffs[id] and self._member_buffs[id].value ~= data.value then
			ShaveHUD:print_log("(HUDList) CompositeBuff:set_value(%s, %s)", tostring(id), tostring(data.value), "info")
	...
	HUDList.TimedInteractionItem.INTERACT_ID_TO_ICON = {
		...
		mask_up 					= { texture = "guis/textures/shavehud/hudlist/mask_up" 				},
		ammo_bag 					= { skills 		= {1, 0}				},
		doc_bag 					= { skills 		= {2, 7}				},
...
if string.lower(RequiredScript) == "lib/managers/objectinteractionmanager" then
	...
	function ObjectInteractionManager:init(...)
		init_original(self, ...)
		if managers.gameinfo and ShaveHUD:getSetting({"HUDSuspicion", "REMOVE_ANSWERED_PAGER_CONTOUR"}, true) then
