if string.lower(RequiredScript) == "lib/managers/hudmanager" then
	...
	Hooks:PostHook( HUDManager , "_player_hud_layout" , "ShaveHUDPostHUDManagerPlayerInfoHUDLayout" , function( self )
		...
		self._unit_health_enemy_location = self._unit_health_panel:text({
			...
			text 		= "^",
			blend_mode 	= "normal",
			visible 	= ShaveHUD:getSetting({"EnemyHealthbar", "SHOW_POINTER"}, false),
			alpha 		= 0.75,
			halign 		= "center",
			font 		= "fonts/font_medium_shadow_mf",
			font_size 	= 20,
	...
	function HUDManager:set_unit_health_visible( visible, shield )
		...
		if visible == true and not self._unit_health_visible and ShaveHUD:getSetting({"EnemyHealthbar", "ENABLED"}, true) then

			self._unit_health_visible = true
			self._unit_health_enemy_location:set_visible(ShaveHUD:getSetting({"EnemyHealthbar", "SHOW_POINTER"}, false))
	...
	function HUDManager:set_unit_health( current , total , tweak_table )
		...
		local enemy = ShaveHUD:getCharacterName(tweak_table, true)
...
elseif string.lower(RequiredScript) == "lib/units/beings/player/states/playerstandard" then
	Hooks:PostHook( PlayerStandard , "_update_fwd_ray" , "ShaveHUDPostPlayerStandardUpdate" , function( self , t , dt )
		if self._fwd_ray and self._fwd_ray.unit and type(self._fwd_ray.unit) == "userdata" then
			...
			if alive( unit ) then
				if unit:in_slot( 25 ) and not unit:character_damage():dead() and (table.contains(managers.groupai:state():turrets() or {}, unit) or ShaveHUD:getSetting({"EnemyHealthbar", "SHOW_CIVILIAN"}, true) and Network:is_server()) then
					self._last_unit = nil
					visible = true
				...
				elseif alive( unit ) and ( unit:in_slot( 12 ) or ShaveHUD:getSetting({"EnemyHealthbar", "SHOW_CIVILIAN"}, false) and ( unit:in_slot( 21 ) or unit:in_slot( 22 ) ) or unit:in_slot( 16 ) and Network:is_server()) and not unit:character_damage():dead() then
				...
				elseif alive( unit ) and unit:in_slot( 39 ) and ShaveHUD:getSetting({"EnemyHealthbar", "SHOW_VEHICLE"}, true) and unit:vehicle_driving() and not self._seat then
					self._last_unit = nil
					visible = true
...
elseif string.lower(RequiredScript) == "lib/states/ingamearrested" then
	Hooks:PostHook( IngameArrestedState , "at_enter" , "ShaveHUDPostIngameArrestedAtEnter" , function( self )
