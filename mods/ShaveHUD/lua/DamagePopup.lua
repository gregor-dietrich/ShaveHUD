if RequiredScript == "lib/units/enemies/cop/copdamage" then
	...
	function CopDamage:_process_popup_damage(data)
		CopDamage.DMG_POPUP_SETTING = ShaveHUD:getSetting({"DamagePopup", "DISPLAY_MODE"}, 2)
	...
	function CopDamage:show_popup(damage, dead, headshot, critical, color_id)
		if managers.waypoints then
			...
			local waypoint_color = color_id and ((color_id == 5 and ShaveHUD:getSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "USE"}, false)) and ShaveHUD:getColorSetting({"CustomHUD", "TEAMMATE", "AI_COLOR", "COLOR"}, Color.white) or tweak_data.chat_colors[color_id]) or ShaveHUD:getColorSetting({"DamagePopup", critical and "CRITICAL_COLOR" or headshot and "HEADSHOT_COLOR" or "COLOR"}, "yellow")
			waypoint_color = waypoint_color:with_alpha(ShaveHUD:getSetting({"DamagePopup", "ALPHA"}, 1))
			local waypoint_duration = ShaveHUD:getSetting({"DamagePopup", "DURATION"}, 3)
			if waypoint and not waypoint:is_deleted() then
				...
				local params = {
					unit = self._unit,
					offset = Vector3(10, 10, ShaveHUD:getSetting({"DamagePopup", "HEIGHT"}, 20)),
					scale = 2 * ShaveHUD:getSetting({"DamagePopup", "SCALE"}, 1),
					...
					icon = {
						type = "icon",
						show = dead,
						scale = ShaveHUD:getSetting({"DamagePopup", "SKULL_SCALE"}, 1.2),
					...
					component_order = { ShaveHUD:getSetting({"DamagePopup", "SKULL_ALIGN"}, 1) == 1 and { "icon", "label" } or { "label", "icon" } , { "duration" } }
				}
