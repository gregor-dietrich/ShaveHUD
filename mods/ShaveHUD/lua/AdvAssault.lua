if string.lower(RequiredScript) == "lib/managers/hud/hudassaultcorner" then
	...
	function HUDAssaultCorner:update_banner_pos()
		...
		local banner_pos = math.clamp(ShaveHUD:getSetting({"AssaultBanner", "POSITION"}, 2), 1, 3)
		local assault_panel = self._hud_panel:child("assault_panel")
...
elseif string.lower(RequiredScript) == "lib/managers/hudmanagerpd2" then
	...
	function HUDManager:_locked_assault(status)
		...
		if self._assault_locked ~= status then
			...
			if Network:is_server() and ShaveHUD.Sync then
				ShaveHUD.Sync:endless_assault_status(self._assault_locked)
			end
		end
	end
...
elseif string.lower(RequiredScript) == "lib/managers/localizationmanager" then
	...
	function LocalizationManager:hud_adv_assault()
		if ShaveHUD:getSetting({"AssaultBanner", "USE_ADV_ASSAULT"}, true) then
			if managers.hud and managers.hud:is_assault_locked() then
				return self:text("shavehud_locked_assault")
			else
				...
				if tweak and gai_state and assault_data and assault_data.active then
					...
					local phase = self:text("shavehud_advassault_phase_title") .. "  " .. self:text("shavehud_advassault_phase_" .. assault_data.phase)

					local spawns = get_value(gai_state, tweak.force_pool) * get_mult(gai_state, tweak.force_pool_balance_mul)
					local spawns_left = self:text("shavehud_advassault_spawns_title") .. "  " .. math.round(math.max(spawns - assault_data.force_spawned, 0))
					...
					--end
					if time_left < 0 then
						time_left = self:text("shavehud_advassault_time_overdue")
					else
						time_left = self:text("shavehud_advassault_time_title") .. "  " .. string.format("%.2f", time_left)
					end
