if not _G.ShaveHUD then
	_G.ShaveHUD = {}
	ShaveHUD.mod_path = ModPath
	ShaveHUD.save_path = SavePath
	ShaveHUD.assets_path = "./assets/mod_overrides/"
	ShaveHUD.settings_path = ShaveHUD.save_path .. "ShaveHUD_v3.json"
	ShaveHUD.tweak_file = "ShaveHUDTweakData.lua"
	ShaveHUD.identifier = string.match(ShaveHUD.mod_path, "[\\/]([%w_%-%.]+)[\\/]$") or "ShaveHUD"

	ShaveHUD.notifications = {}
	ShaveHUD.settings = {}
	ShaveHUD.tweak_data = {}

	function ShaveHUD:Reset()
		...
		ShaveHUD.settings = {
			...
			GADGETS = {
				...
				SHOW_ANGLED_SIGHT						= true,
			},
			EQUIPMENT = {
				AUTO_DISCARD_PARACHUTE					= true,
				SENTRY_AUTO_AP 							= true,
				ECM_FEEDBACK_STEALTH_DISABLED			= true,
				SHAPED_CHARGE_STEALTH_DISABLED			= true,
				KEYCARD_DOORS_DISABLED					= true,
				ENABLE_BURSTMODE						= false,
				ENABLE_AKIMBO_MODE						= true,
			},
			INVENTORY = {
				BIGGER_BETTER_REPLACED					= true,
				CLEAR_DESCRIPTIONS						= true,
				CORRECT_ARMOR_SORTING					= true,
				INVENTORY_COUNT							= true,
				MASK_IN_OUTFIT_SCREEN					= true,
				NO_WEAPON_ROTATION						= true,
				RETURNABLE_MASK_COMPONENTS				= true,
			...
			SkipIt = {
				SKIP_INTROS		 						= true,		--Skip the intros on game start
				SKIP_BLACKSCREEN 						= false,	--Skip the blackscreen on mission start
				INSTANT_CARDFLIP						= true,		--Skip the countdown before flipping a chosen card
				INSTANT_RESTART							= true,
				STAT_SCREEN_SPEEDUP						= true,
				STAT_SCREEN_DELAY 						= 0,		--Skip the experience screen after X seconds
				AUTOPICK_CARD 							= true,		--Automatically pick a card on lootscreen
				AUTOPICK_CARD_SPECIFIC 					= 2,		--left, center, right, random
				LOOT_SCREEN_DELAY 						= 1.5,		--Skip the loot screen after X seconds
				NO_SLOWMOTION 							= true,		--Disable mask-up and downed slow motion
			},
			Fixes = {
				ABILITY_USE								= true,
				AUTO_FIRE_SOUND							= true,
				BOT_BULLET_COLLISION					= true,
				BULLET_PUSH								= true,
				CLOAKER_CAM_LOCK						= true,
				CORRECT_DIFF_ENEMIES					= false,
				COUNTER_TASER							= true,
				CRIMESPREE_CRASH_LOSS					= true,
				CRIMESPREE_PHALANX_SPAWN_LIMIT			= true,
				CRIMESPREE_RANKSPREAD_FILTER			= true,
				DIFF_CHECK_FALLBACK						= false,
				DOZER_SPRINT							= true,
				KEEP_HEIST_OUTFITS						= true,
				LOGICAL_HEIST_OUTFITS					= true,
				--TO DO MAG_DROP								= true,
				RELOAD_SPEED_BOOST						= true,
				SHOTGUN_RELOAD							= true,
				STALE_LOBBY								= true,
			...
			Misc = {
				...
				NO_SWAY									= true,
				PLAYER_BLOOD							= true,
				REDUNDANT_CARD_REMOVAL					= true,
				SHIELD_DESPAWN_TIMER					= true,
				SHIELD_DESPAWN_TIMER_VALUE				= 3,
				STARRING								= true,
				VEHICLE_FOV								= true,
				VEHICLE_FOV_VALUE						= 90,
			},
			MOD_OVERRIDES = {
				fed_inv									= true,
				hawks_soundpack							= true,
				shavehud_assets							= true,
				shavehud_extras							= true,
			},
			LOBBY_SETTINGS = {
				job_plan = -1,
				kick_option = 1,
		...
		if msg_type and LOG_MODES[tostring(msg_type)] then
			if type(text) == "table" or type(text) == "userdata" then
				...
				if not text[1] or type(text[1]) ~= "string" then
					log(string.format("[ShaveHUD] %s:", string.upper(type(msg_type))))
					log_table(text)
					return
				else
			...
			text = string.format("[ShaveHUD] %s: %s", string.upper(msg_type), text)
			log(text)
	...
	function ShaveHUD:createDirectory(path)
		local current = ""
	...
	function ShaveHUD:DirectoryExists(path)
	...
	function ShaveHUD:getVersion()
        local mod = BLT and BLT.Mods:GetMod(ShaveHUD.identifier or "")
		return tostring(mod and mod:GetVersion() or "(n/a)")
	end

	function ShaveHUD:SafeTableConcat(tbl, str)
		local res
		for i = 1, #tbl do
	...
	function ShaveHUD:getSetting(id_table, default)
	...
	function ShaveHUD:setSetting(id_table, value)
		local entry = self.settings
	...
	function ShaveHUD:getColor(name, ...)
		if self.tweak_data and type(name) == "string" then
			for i, data in ipairs(self:getTweakEntry("color_table", "table")) do
				if name == data.name then
	...
	function ShaveHUD:getTweakEntry(id, val_type, default)
	...
	function ShaveHUD:getCharacterName(character_id, to_upper)
	...
	function ShaveHUD:truncateNameTag(name)
	...
	if not ShaveHUD.tweak_path then		-- Populate tweak data
		local tweak_path = string.format("%s%s", ShaveHUD.save_path, ShaveHUD.tweak_file)
		if not io.file_is_readable(tweak_path) then
			tweak_path = string.format("%s%s", ShaveHUD.mod_path, ShaveHUD.tweak_file)
		end
		if io.file_is_readable(tweak_path) then
			dofile(tweak_path)
			ShaveHUD.tweak_data = ShaveHUDTweakData:new()
		else
			ShaveHUD:print_log(string.format("Tweak Data file couldn't be found! (%s)", tweak_path), "error")
		end
	end

	-- Table with all menu IDs
	ShaveHUD.menu_ids = ShaveHUD.menu_ids or {}

	--callback functions to apply changed settings on the fly
	if not ShaveHUD.apply_settings_clbk then
		ShaveHUD.apply_settings_clbk = {
			["CustomHUD"] = function(setting, value)
				if managers.hud and managers.hud.change_hud_setting then
					...
					managers.hud:change_hud_setting(type, setting, ShaveHUD:getColor(value) or value)
				end
			end,
			["HUDList"] = function(setting, value)
				if managers.hud and HUDListManager and setting then
					...
					if list == "BUFF_LIST" and category ~= "show_buffs" then
						managers.hud:change_bufflist_setting(option, ShaveHUD:getColor(value) or value)
					elseif list == "RIGHT_LIST" and category == "SHOW_PICKUP_CATEGORIES" then
						managers.hud:change_pickuplist_setting(option, ShaveHUD:getColor(value) or value)
					else
						managers.hud:change_list_setting(option, ShaveHUD:getColor(value) or value)
					end
				end
			end,
			...
			["DrivingHUD"] = function(setting, value)
				if managers.hud and managers.hud.change_drivinghud_setting then
					managers.hud:change_drivinghud_setting(tostring(setting[#setting]), ShaveHUD:getColor(value) or value)
				end
			end,
			["GADGETS"] = function(setting, value)
				if WeaponGadgetBase and WeaponGadgetBase.update_theme_setting and #setting >= 4 then
					WeaponGadgetBase.update_theme_setting(setting[1], setting[2], setting[3], setting[4], ShaveHUD:getColor(value) or value)
				end
			end,
			["MOD_OVERRIDES"] = function(setting, value)
				...
				if ShavehudAssets then
					ShavehudAssetsAssets.loadAssetsCallback()
				end
			end
		}
	end

	if not ShaveHUD:DirectoryExists(ShaveHUD.assets_path) then
		ShaveHUD:print_log("Folder '%s' doesn't exist, creating....\t%s", ShaveHUD.assets_path, tostring(ShaveHUD:createDirectory(ShaveHUD.assets_path)), "warining")
	end

	ShaveHUD:Reset()	-- Populate settings table
	ShaveHUD:Load()	-- Load user settings

	do	-- Romove Disabled Updates, so they don't show up in the download manager.
		local mod = BLT and BLT.Mods:GetMod(ShaveHUD.identifier or "")
		for i, update in pairs(mod:GetUpdates()) do
			if update:GetInstallFolder() ~= ShaveHUD.identifier then
				local directory = Application:nice_path( update:GetInstallDirectory() .. "/" .. update:GetInstallFolder(), true )
				if ShaveHUD:getSetting({"MOD_OVERRIDES", update:GetId()}) then
					ShaveHUD:createDirectory(directory)
				else
	...
	-- Create Ingame Menus
	do
		dofile(ShaveHUD.mod_path .. "OptionMenus.lua")	-- Menu structure table in seperate file, in order to not bloat the Core file too much.
		local menu_options = ShaveHUD.options_menu_data

		-- Setup and register option menus
		Hooks:Add("MenuManagerSetupCustomMenus", "MenuManagerSetupCustomMenus_ShaveHUD", function( menu_manager, nodes )
		...
		Hooks:Add("MenuManagerPopulateCustomMenus", "MenuManagerPopulateCustomMenus_ShaveHUD", function(menu_manager, nodes)
			-- Called on setting change
			local function change_setting(setting, value)
				if ShaveHUD:getSetting(setting, nil) ~= value and ShaveHUD:setSetting(setting, value) then
					ShaveHUD:print_log(string.format("Change setting: %s = %s", ShaveHUD:SafeTableConcat(setting, "->"), tostring(value)), "info")	-- Change type back!
					ShaveHUD.settings_changed = true

					local script = table.remove(setting, 1)
					if ShaveHUD.apply_settings_clbk[script] then
						ShaveHUD.apply_settings_clbk[script](setting, value)
					end
				end
			end
				...
				MenuCallbackHandler[visual_clbk_id] = function(self, item)
					for _, req in ipairs(data.visible_reqs or {}) do
						if type(req) == "table" then
							local a = ShaveHUD:getSetting(req.setting, nil)
							if req.equal then
								if a ~= req.equal then
									return false
								end
			...
			local update_visible_clbks = "shavehud_update_visibility"
			...
			local create_item_handlers = {
				menu = function(parent_id, offset, data)
					if not table.contains(ShaveHUD.menu_ids, data.menu_id) then
						table.insert(ShaveHUD.menu_ids, data.menu_id)
					end
				end,
				...
				multi_choice = function(menu_id, offset, data, value)
					...
					if data.add_color_options then
						...
						for i, item in pairs(menu._items_list) do
							if item:parameters().name == id then
								item:clear_options()
								for k, v in ipairs(ShaveHUD:getTweakEntry("color_table", "table") or {}) do
									if data.add_rainbow or v.name ~= "rainbow" then
										local color_name = managers.localization:text("shavehud_colors_" .. v.name)
			...
			local function populate_menu(menu_table, parent_id)
				...
				for i, data in ipairs(menu_table) do
					local value = data.value and ShaveHUD:getSetting(data.value, nil)
		...
		Hooks:Add("MenuManagerBuildCustomMenus", "MenuManagerBuildCustomMenus_ShaveHUD", function(menu_manager, nodes)
			local back_clbk = "shavehud_back_clbk"
			local focus_clbk = "shavehud_focus_clbk"
			local reset_clbk = "shavehud_reset_clbk"

			-- Add menu back callback
			MenuCallbackHandler[back_clbk] = function(node)
				if ShaveHUD.settings_changed then
					ShaveHUD.settings_changed = nil
					ShaveHUD:Save()
					ShaveHUD:print_log("Settings saved!", "info")
				end
			end
			...
			MenuCallbackHandler[reset_clbk] = function(self, item)
				local menu_title = managers.localization:text("shavehud_reset_options_title")
				local menu_message = managers.localization:text("shavehud_reset_options_confirm")
				local menu_buttons = {
					[1] = {
						...
						callback = function(self, item)
							...
							for i, menu_id in ipairs(ShaveHUD.menu_ids) do
								local menu = MenuHelper:GetMenu(menu_id)
								if menu then
									for __, menu_item in ipairs(menu._items_list) do
										...
										if menu_item.set_value and setting then
											local value = ShaveHUD:getSetting(setting, nil)
											if value ~= nil then
												local script = table.remove(setting, 1)
												if ShaveHUD.apply_settings_clbk[script] then
													ShaveHUD.apply_settings_clbk[script](setting, value)
							...
							ShaveHUD.settings_changed = true
							ShaveHUD:print_log("Settings resetted!", "info")
						end,
					},
					[2] = {
	...
	Hooks:Add("LocalizationManagerPostInit", "LocalizationManagerPostInit_ShaveHUD", function(loc)
        local loc_path = ShaveHUD.mod_path .. "loc/"
		if file.DirectoryExists( loc_path ) then
			loc:load_localization_file(string.format("%s/%s.json", loc_path, ShaveHUD:getSetting({"LANGUAGE"}, "english")))
			loc:load_localization_file(string.format("%s/english.json", loc_path), false)

			if ShaveHUD:getSetting({"INVENTORY", "USE_REAL_WEAPON_NAMES"}, false) then
				loc:load_localization_file(string.format("%s/RealWeaponNames.json", loc_path))
			else
				local text_original = LocalizationManager.text
				local testAllStrings = false
				function LocalizationManager:text(string_id, ...)
				return string_id == "bm_w_amcar"    and "AMCAR"
				or string_id == "bm_w_m4"       and "CAR-4"
				or string_id == "bm_w_ak74"     and "AK"
				or string_id == "bm_w_m14"      and "M308"
				or string_id == "bm_w_aug"      and "UAR"
				or string_id == "bm_w_akm"      and "AK.762"
				or string_id == "bm_w_akm_gold" and "Golden AK.762"
				or string_id == "bm_w_g36"      and "JP36"
				or string_id == "bm_w_ak5"      and "AK5"
				or string_id == "bm_w_m16"      and "AMR-16"
				or string_id == "bm_w_s552"     and "Commando 553"
				or string_id == "bm_w_scar"     and "Eagle Heavy"
				or string_id == "bm_w_fal"      and "Falcon"
				or string_id == "bm_w_famas"    and "Clarion"
				or string_id == "bm_w_galil"    and "Gecko 7.62"
				or string_id == "bm_w_g3"       and "Gewehr 3"
				or string_id == "bm_w_l85a2"    and "Queen's Wrath"
				or string_id == "bm_w_vhs"      and "Lion's Roar"
				or string_id == "bm_w_asval"    and "Valkyria"
				or string_id == "bm_w_sub2000"  and "Cavity 9mm"
				or string_id == "bm_w_tecci"    and "Bootleg"
				...
				or string_id == "bm_w_ksg"      and "Raven"
				or string_id == "bm_w_aa12"     and "Steakout 12G"
				or string_id == "bm_w_boot"     and "Breaker 12G"
				or string_id == "bm_w_rota"     and "Goliath 12G"
				or string_id == "bm_w_jowi"     and "Akimbo Chimano Compact"
				or string_id == "bm_w_x_1911"   and "Akimbo Crosskill"
				or string_id == "bm_w_x_b92fs"  and "Akimbo Bernetti 9"
				or string_id == "bm_w_x_deagle" and "Akimbo Deagle"
				or string_id == "bm_w_x_g17"    and "Akimbo Chimano 88"
				or string_id == "bm_w_x_g22c"   and "Akimbo Chimano Custom"
				or string_id == "bm_w_x_usp"    and "Akimbo Interceptor 45"
				or string_id == "bm_w_x_packrat" and "Akimbo Contractor"
				or string_id == "bm_w_gre_m79"          and "GL40"
				or string_id == "bm_w_saw"              and "OVE9000"
				or string_id == "bm_w_m134"             and "Vulcan Minigun"
				or string_id == "bm_w_m32"              and "Piglet"
				or string_id == "bm_w_flamethrower_mk2" and "Flamethrower"
				or string_id == "bm_w_plainsrider"      and "Plainsrider Bow"
				or string_id == "bm_w_msr"            and "Rattlesnake"
				or string_id == "bm_w_r93"            and "R93"
				or string_id == "bm_w_m95"            and "Thanatos .50 cal"
				or string_id == "bm_w_mosin"          and "Nagant"
				or string_id == "bm_w_winchester1874" and "Repeater 1874"
				or string_id == "bm_w_wa2000"         and "Lebensauger .308"
				or string_id == "bm_w_model70"        and "Platypus 70"
				or string_id == "bm_w_desertfox"     and "Desertfox"
				or string_id == "bm_w_tti" and "Contractor .308"
				or string_id == "bm_w_rpk"  and "RPK"
				or string_id == "bm_w_m249" and "KSP"
				or string_id == "bm_w_hk21" and "Brenner 21"
				or string_id == "bm_w_mg42" and "Buzzsaw 42"
				or string_id == "bm_w_par"  and "Ksp 58"
				or string_id == "bm_w_usp"         and "Interceptor 45"
				or string_id == "bm_w_g22c"        and "Chimano Custom"
				or string_id == "bm_wp_pis_g26"    and "Chimano Compact"
				or string_id == "bm_w_glock_17"    and "Chimano 88"
				or string_id == "bm_w_colt_1911"   and "Crosskill"
				or string_id == "bm_w_b92fs"       and "Bernetti 9"
				or string_id == "bm_w_raging_bull" and "Bronco .44"
				or string_id == "bm_w_glock_18c"   and "STRYK 18c"
				or string_id == "bm_w_deagle"      and "Deagle"
				or string_id == "bm_w_ppk"         and "Gruber Kurz"
				or string_id == "bm_w_p226"        and "Signature .40"
				or string_id == "bm_w_c96"         and "Broomstick"
				or string_id == "bm_w_hs2000"      and "LEO"
				or string_id == "bm_w_peacemaker"  and "Peacemaker .45"
				or string_id == "bm_w_mateba"      and "Matever .357"
				or string_id == "bm_w_sparrow"     and "Baby Deagle"
				or string_id == "bm_w_pl14"        and "White Streak"
				or string_id == "bm_w_packrat"	   and "Contractor"
				or string_id == "bm_w_m1928"    and "Chicago Typewriter"
				or string_id == "bm_w_mac10"    and "Mark 10"
				or string_id == "bm_w_mp5"      and "Compact-5"
				or string_id == "bm_w_mp9"      and "CMP"
				or string_id == "bm_w_olympic"  and "Para"
				or string_id == "bm_w_akmsu"    and "Krinkov"
				or string_id == "bm_w_p90"      and "Kobus 90"
				or string_id == "bm_w_m45"      and "Swedish K"
				or string_id == "bm_w_mp7"      and "SpecOps"
				or string_id == "bm_w_scorpion" and "Cobra"
				or string_id == "bm_w_tec9"     and "Blaster 9mm"
				or string_id == "bm_w_uzi"      and "Uzi"
				or string_id == "bm_w_sterling" and "Pachet L2A1"
				or string_id == "bm_w_cobray"   and "Jacket's Piece"
				or string_id == "bm_w_polymer"  and "Kross Vertex"
				or string_id == "bm_w_baka"     and "Micro Uzi"
				or string_id == "bm_w_x_sr2"    and "Akimbo Heather"
				or string_id == "bm_w_sr2"      and "Heather"
				or string_id == "bm_w_x_mp5"    and "Akimbo Compact-5"
				or string_id == "bm_w_x_akmsu"  and "Akimbo Krinkov"
				or string_id == "bm_w_hajk"     and "CR 805B"
				or string_id == "bm_w_schakal"  and "Jackal"
				or string_id == "bm_w_judge"   and "The Judge"
				or string_id == "bm_w_serbu"   and "Locomotive 12G"
				or string_id == "bm_w_striker" and "Street Sweeper"
				or string_id == "bm_w_m37"     and "GSPS 12G"
				or string_id == "bm_w_rpg7"   and "HRL-7"
				or string_id == "bm_w_hunter" and "Pistol Crossbow"
				or string_id == "bm_w_china"  and "China Puff 40mm"
				or string_id == "bm_w_arbiter"and "Arbiter"
				or string_id == "bm_w_ray"    and "Commando 101"
				or string_id == "bm_w_ak12"  and "AK17"
				or string_id == "bm_w_coal"   and "Tatonka"
				or string_id == "bm_w_siltstone" and "Grom"
				or string_id == "bm_w_lemming" and "5/7 AP"
				or string_id == "bm_w_chinchilla" and "Castigo .44"
				or string_id == "bm_w_x_chinchilla" and "Akimbo Castigo .44"
				or string_id == "bm_w_ching" and "Galant"
				or string_id == "bm_w_breech" and "Parabellum"
				or string_id == "bm_w_x_breech" and "Akimbo Parabellum"
				or string_id == "bm_w_erma" and "MP40"
				or string_id == "bm_w_x_erma" and "Akimbo MP40"
				or string_id == "bm_w_corgi" and "Union 5.56"
				or string_id == "bm_w_komodo" and "Tempest-21"
				or string_id == "bm_w_m1897" and "Reinfeld 88"
				or string_id == "bm_w_m60" and "M60"
				or string_id == "bm_w_r700" and "R700"
				or string_id == "bm_w_sbl" and "Bernetti Rangehitter"
				or string_id == "bm_w_x_p226" and "Akimbo Signature .40"
				or string_id == "bm_w_x_ppk" and "Akimbo Gruber Kurz"
				or string_id == "bm_w_x_rage" and "Akimbo Bronco .44"
				or string_id == "bm_w_x_p114" and "Akimbo White Streak"
				or string_id == "bm_w_x_sparrow" and "Akimbo Baby Deagle"
				or string_id == "bm_w_legacy" and "M13 9mm"
				or string_id == "bm_w_x_legacy" and "Akimbo M13 9mm"
				or string_id == "bm_w_x_c96" and "Akimbo Broomstick"
				or string_id == "bm_w_model3" and "Frenchman Model 87"
				or string_id == "bm_w_x_model3" and "Akimbo Frenchman Model 87"
				or string_id == "bm_w_shrew" and "Crosskill Guard"
				or string_id == "bm_w_x_shrew" and "Akimbo Crosskill Guard"
				or string_id == "bm_w_x_hs2000" and "Akimbo LEO"
				or string_id == "bm_w_x_g18c" and "Akimbo STRYK 18c"
				or string_id == "bm_w_beer" and "Bernetti Auto"
				or string_id == "bm_w_x_beer" and "Akimbo Bernetti Auto"
				or string_id == "bm_w_czech" and "Czech 92"
				or string_id == "bm_w_x_czech" and "Akimbo Czech 92"
				or string_id == "bm_w_stech" and "Igor Automatik"
				or string_id == "bm_w_x_stech" and "Akimbo Igor Automatik"
				or string_id == "bm_w_holt" and "HOLT 9mm"
				or string_id == "bm_w_x_holt" and "Akimbo HOLT 9mm"
				or string_id == "bm_w_x_2006m" and "Akimbo Matever .357"
				or string_id == "bm_w_shepheard" and "Signature"
				or string_id == "bm_w_x_shepheard" and "Akimbo Signature"
				or string_id == "bm_w_coach" and "Claire 12G"
				or string_id == "bm_w_basset" and "Grimm 12G"
				or string_id == "bm_w_x_basset" and "Brothers Grimm 12G"
				or string_id == "bm_w_x_judge" and "Akimbo Judge"
				or string_id == "bm_w_x_rota" and "Akimbo Goliath 12G"
				or string_id == "bm_w_slap" and "Compact 40mm"
				or string_id == "bm_w_x_m45" and "Akimbo Swedish K"
				or string_id == "bm_w_x_mp7" and "Akimbo SpecOps"
				or string_id == "bm_w_x_hajk" and "Akimbo CR 805B"
				or string_id == "bm_w_x_mac10" and "Akimbo Mark 10"
				or string_id == "bm_w_x_m1928" and "Akimbo Chicago Typewriter"
				or string_id == "bm_w_x_scorpion" and "Akimbo Cobra"
				or string_id == "bm_w_x_baka" and "Akimbo Micro Uzi"
				or string_id == "bm_w_x_mp9" and "Akimbo CMP"
				or string_id == "bm_w_x_olympic" and "Akimbo Para"
				or string_id == "bm_w_x_schakal" and "Akimbo Jackal"
				or string_id == "bm_w_x_tec9" and "Akimbo Blaster 9mm"
				or string_id == "bm_w_x_p90" and "Akimbo Kobus 90"
				or string_id == "bm_w_x_polymer" and "Akimbo Kross Vertex"
				or string_id == "bm_w_x_coal" and "Akimbo Tatonka"
				or string_id == "bm_w_x_sterling" and "Akimbo Patchett L2A1"
				or string_id == "bm_w_x_uzi" and "Akimbo Uzi"
		...
		localized_strings["cash_sign"] = ShaveHUD:getTweakEntry("CASH_SIGN", "string", "$")

		-- Hide Skip Message, when auto skip blackscreen is active
		if ShaveHUD:getSetting({"SkipIt", "SKIP_BLACKSCREEN"}, false) then
	...
	Hooks:Add("MenuManagerOnOpenMenu", "MenuManagerOnOpenMenu_ShaveHUDCore", function(menu_manager, menu_name, position)
		if menu_name == "menu_main" then
			local mod = BLT and BLT.Mods:GetMod(ShaveHUD.identifier or "")

			if mod and not mod.supermod then
				QuickMenu:new( 
					managers.localization:text("shavehud_dialog_requires_superblt_title"), 
					managers.localization:text("shavehud_dialog_requires_superblt_desc_long", {URL = "https://superblt.znix.xyz"}), 
					{
				...
				if BLT.Notifications then
					local params = {
						title = managers.localization:text("shavehud_dialog_requires_superblt_title"),
						text = managers.localization:text("shavehud_dialog_requires_superblt_desc_short", {URL = "superblt.znix.xyz"}),
					...
					ShaveHUD.notifications["no_superblt"] = notif_id
				end
			elseif mod:AreUpdatesEnabled() and ShaveHUD.identifier ~= "ShaveHUD" then
				if BLT.Notifications then
					local params = {
						title = managers.localization:text("shavehud_dialog_update_install_folder_title"),
						text = managers.localization:text("shavehud_dialog_update_install_folder_desc", {FOLDER = ShaveHUD.identifier}),
						icon = mod:GetModImage(),
						--icon_texture_rect = {0, 0, 1 , 1},
						color = Color(1, 0.2, 0),
						priority = 10
					...
					local notif_id = BLT.Notifications:add_notification(params)
					ShaveHUD.notifications["no_autoupdates"] = notif_id
				end
			end
		end
	end)
